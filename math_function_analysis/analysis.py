...
class MathFunctionAnalysis:
    ...
    def display_report(self):
        # Display Domain
        domain_text = "\\textbf{{{}}}: {}".format(
            self._translate('Domain'), sp.latex(self.domain)
        )
        display(Latex(domain_text))

        # Display Intercepts
        intercepts_text = "\\textbf{{{}}}: X: {}, Y: {}".format(
            self._translate('Intercepts'),
            ', '.join(map(lambda x: sp.latex(x), self.intercepts['x'])),
            sp.latex(self.intercepts['y'])
        )
        display(Latex(intercepts_text))

        # Display Asymptotes
        asymptotes_text = "\\textbf{{{}}}: {}: {}, {}: {}, {}: {}".format(
            self._translate('Asymptotes'),
        ...
        display(Latex(asymptotes_text))

        # Display First Derivative Analysis
        first_derivative_analysis_text = (
            "\\textbf{{{}}}: "
            "{}: {}, {}: {}, {}: {}, {}: {}, {}: {}"
        ).format(
            self._translate('First Derivative Analysis'),
        ...
        display(Latex(first_derivative_analysis_text))

        # Display Second Derivative Analysis
        second_derivative_analysis_text = (
            "\\textbf{{{}}}: "
            "{}: {}, {}: {}, {}: {}, {}: {}"
        ).format(
            self._translate('Second Derivative Analysis'),
            ...
            ', '.join(map(lambda x: "${}$: ${}$".format(sp.latex(x), sp.latex(
                self.second_derivative_analysis['inflection_values'][x])),
                self.second_derivative_analysis['inflection_values']))
        )
        display(Latex(second_derivative_analysis_text))
